(function ($, window, document) {
    ...
    $.extend(Plugin.prototype, {
        init: function () {
            ...
            var base = this, $template = $("<div>", {id: "floatingSocialShare"}), $child = $("<div>", {class: this.settings.place}).appendTo($template);
            $.each(this.settings.buttons, function (index, value) {
                $.each(networks, function (k, v) {
                    if (value === k) {
                        var $icon = $("<i>", {class: "margin-top-5 fa fa-" + value}), _href = v.url.replace('{url}', base.settings.url).replace('{title}', base.settings.title).replace('{description}', base.settings.description).replace('{media}', base.settings.media), $component = $("<a>", {
                            title: base.settings.title,
                            class: v.className + " pop-upper"
                        }).attr("href", _href).attr("title", base.settings.text + value).append($icon);
                        if (base.settings.counter === !0) {
    ...
    var places = ["content-left", "top-left", "top-right"], networks = {
        "envelope": {
            className: "envelope",
    ...
    function setMobileCss(objects) {
        ...
        $.each(objects, function () {
            if (w < 961)
                $(this).css("width", 100 / objects.length + "%"); else $(this).removeAttr("style")
        })
    }
    ...
    function setShareCount(network, url, $component, twitter_counter) {
        switch (network) {
            ...
            case"linkedin":
                ...
                break;
            case"odnoklassniki":
                $.get('https://connect.ok.ru/dk?st.cmd=extLike&ref=' + url + '&callback=?', function () {
                }, 'jsonp');
            ...
            case"twitter":
                ...
                break;
            case"vk":
                $.get('https://vk.com/share.php?act=count&index=1&url=' + url + '&callback=?', function () {
                }, 'jsonp');
                window.VK = window.VK || {};
